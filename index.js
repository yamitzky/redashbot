...
if (
  !(
    (process.env.REDASH_HOST && process.env.REDASH_API_KEY) ||
    process.env.REDASH_HOSTS_AND_API_KEYS
  )
) {
  console.error(
    'Error: Specify REDASH_HOST and REDASH_API_KEY in environment values'
  )
  console.error(
    'Or you can set multiple Redash configs by specifying like below'
  )
  console.error(
...
const parseApiKeysPerHost = () => {
  if (process.env.REDASH_HOST) {
    ...
    return process.env.REDASH_HOSTS_AND_API_KEYS.split(',').reduce(
      (m, host_and_key) => {
...
const slackMessageEvents =
  process.env.SLACK_MESSAGE_EVENTS || DEFAULT_SLACK_MESSAGE_EVENTS
...
const takeScreenshot = async (url) => {
  ...
  await page.goto(url)
  await sleep(
    process.env.SLEEP_TIME ? parseFloat(process.env.SLEEP_TIME) : 2000
  )
...
const uploadFile = async (channel, filename, file) => {
  ...
  await request.post({
    url: 'https://api.slack.com/api/files.upload',
    formData: options,
    simple: true,
  })
}
...
Object.keys(redashApiKeysPerHost).forEach((redashHost) => {
  ...
  controller.hears(
    ...
    slackMessageEvents,
    faultTolerantMiddleware(async (bot, message) => {
